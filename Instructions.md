...
## TA Quick Start (Virtual Environment)
...
### 4) Run the full 9-input pipeline

```powershell
python main.py --all --project-input project-yamls.zip
```

If `project-yamls.zip` is not present, use:

```powershell
python main.py --all --project-input YAMLfiles
```

### 5) Run a single input pair
...
- KDE YAML files: `outputs/<pdf-name>/<pdf-name>-kdes.yaml`
- Comparator outputs: `outputs/comparator/`
- Executor outputs: `outputs/executor/`
